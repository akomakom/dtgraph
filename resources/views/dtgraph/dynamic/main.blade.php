...
<script>
  ...
  var Line = function (serialNumber, cssClass, color) {
  ...
  var Lines = function () {
    ...
    this.addLine = function (serialNumber, cssClass, color) {
      if (this.list[cssClass]) {
        console.log("Already have a line with class " + cssClass);
        throw "Already have line " + cssClass;
      }

      this.list[cssClass] = new Line(serialNumber, cssClass, color);
...
//                .attr("d", function (d) { return line(d.values)});
    ...
    this.updateData = function (lineWrapper, postcallback, setXDomain) {
      ...
      d3.json(url, function (error, data) {
        ...
        if (lineWrapper.path == null) {
          ...
          .attr("d", line);
          if (lineWrapper.color) { //override default class color
            path.attr("style", "stroke: " + lineWrapper.color);
          }
    ...
    this.addLine = function (serialNumber, cssClass, color, graphMode) {
      var result;
//
...
//                line = new Line(serialNumber, cssClass, null, null);
      try {
        var line = lines.addLine(serialNumber, cssClass, color);
//            }
...
//                        setTimeout(applyAlreadyCheckedSensorCheckboxes, 0);
      ...
      $scope.applyCheckedSensors = function () {
        ...
        $.each($scope.$storage.checkedSensors, function (serialNumber, onState) {
          ...
          // find metadata for this sensor so we can find its color
          var metadata = $.grep($scope.sensors, function (item, index) {
          ...
          $.each($scope.$storage.graphModes, function (mode, modeOn) {
            if (onState && modeOn) {
              gm.addLine(serialNumber, cssClass + " " + mode, color, mode);
            } else {
